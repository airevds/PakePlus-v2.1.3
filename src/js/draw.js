    ...
    container.innerHTML = `
        <div class="card" id="setupCard">
            ...
            <div class="form-group">
                ...
                <select id="specifiedExpertSelect">
...
function loadTechnicalExpertsForSelection() {
    ...
    // 添加专家选项
    technicalExperts.forEach(expert => {
...
function renderDrawResults() {
    ...
    let html = `
        <div class="alert alert-info">
            ...
            <small>注意：被拒绝的专家将保留在抽签记录中，系统会抽取新的替代专家。</small>
        </div>
        ...
        html += `
            <div style="margin: 20px 0; padding: 15px; background: #fdedec; border-radius: 8px;">
                <h4 style="margin: 0 0 10px 0; color: #c0392b;">
                    <i class="fas fa-history"></i> 技术组被拒绝专家 (${drawState.history.technical.length}位)
                </h4>
    ...
    if (drawState.history.supervisor.length > 0) {
        html += `
            <div style="margin: 20px 0; padding: 15px; background: #fdedec; border-radius: 8px;">
                <h4 style="margin: 0 0 10px 0; color: #c0392b;">
                    <i class="fas fa-history"></i> 监督组被拒绝专家 (${drawState.history.supervisor.length}位)
                </h4>
                <div style="display: flex; flex-wrap: wrap; gap: 10px;">
...
function exportDrawResults() {
    ...
    exportData.push({
        ...
        '科室': `技术组人数: ${drawState.technicalNeeded}`,
        '类型': `监督组人数: ${drawState.supervisorNeeded}`,
        '是否参加': '状态',
        '备注': '备注'
    });
...
// 在 draw.js 的 exportDrawResults 函数后添加：
...
function exportDrawResultsToPDF() {
    ...
    try {
        ...
        const pdfContent = `
            <div id="pdfContent">
                <div style="text-align: center; margin-bottom: 30px; border-bottom: 2px solid #3498db; padding-bottom: 20px;">
                    <h1 style="color: #2c3e50; margin-bottom: 10px;">专家抽签结果报告</h1>
                    <div style="font-size: 18px; color: #7f8c8d;">${drawState.projectName}</div>
                </div>
                
                <div style="margin-bottom: 30px; background: #f8f9fa; padding: 20px; border-radius: 8px;">
                    <h2 style="color: #2c3e50; margin-bottom: 15px; font-size: 20px;">抽签信息</h2>
                    <table style="width: 100%; border-collapse: collapse;">
                        <tr>
                            <td style="padding: 8px 0; width: 120px; color: #1d1d1d;"><strong>项目名称：</strong></td>
                        ...
                        <tr>
                            ...
                            <td style="padding: 8px 0;">${confirmedTechnical.length} 位确认参加，${rejectedTechnical.length} 位无法参加</td>
                        </tr>
                        <tr>
                            <td style="padding: 8px 0; color: #555;"><strong>监督组专家：</strong></td>
                            <td style="padding: 8px 0;">${confirmedSupervisor.length} 位确认参加，${rejectedSupervisor.length} 位无法参加</td>
                        </tr>
                        <tr>
                            <td style="padding: 8px 0; color: #555;"><strong>优先抽取组别：</strong></td>
                ...
                <div style="margin-bottom: 30px;">
                    <h2 style="color: #27ae60; margin-bottom: 15px; font-size: 20px; padding-bottom: 10px; border-bottom: 1px solid #eee;">
                        <i class="fas fa-check-circle"></i> 确认参加的专家
                    </h2>
                    
                    <h3 style="color: #2c3e50; margin: 20px 0 10px; font-size: 18px;">技术组专家（${confirmedTechnical.length}位）</h3>
                    ${confirmedTechnical.length > 0 ? `
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; border: 1px solid #ddd;">
                        <thead style="background: #2c3e50; color: white;">
                            <tr>
                                <th style="padding: 8px; text-align: left; width: 10%;">专家编号</th>
                                <th style="padding: 8px; text-align: left; width: 10%;">姓名</th>
                                <th style="padding: 8px; text-align: left; width: 25%;">科室</th>
                                <th style="padding: 8px; text-align: left; width: 25%;">类型</th>
                                <th style="padding: 8px; text-align: left; width: 20%;">联系电话</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${confirmedTechnical.map((expert, index) => `
                                <tr style="${index % 2 === 0 ? 'background: #f9f9f9;' : ''}">
                                    <td style="padding: 10px; border-bottom: 1px solid #eee;">
                                        ${expert.name} ${drawState.specifiedExpert && expert.id === drawState.specifiedExpert.id ? '<span style="color: #f39c12; font-size: 12px;">(指定)</span>' : ''}
                                    </td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee; color: #7f8c8d; font-size: 14px;">${expert.id}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.department}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.type}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.phone}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
                    ` : '<p style="color: #7f8c8d; text-align: center; padding: 20px;">无确认参加的技术组专家</p>'}
                    
                    <h3 style="color: #2c3e50; margin: 20px 0 10px; font-size: 18px;">监督组专家（${confirmedSupervisor.length}位）</h3>
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; border: 1px solid #ddd;">
                        ...
                        <tbody>
                            ${confirmedSupervisor.map((expert, index) => `
                                <tr style="${index % 2 === 0 ? 'background: #f9f9f9;' : ''}">
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.name}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee; color: #7f8c8d; font-size: 14px;">${expert.id}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.department}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.type}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.phone}</td>
                    ...
                    ` : '<p style="color: #7f8c8d; text-align: center; padding: 20px;">无确认参加的监督组专家</p>'}
                </div>
                
                <!-- 无法参加的专家 -->
                ${(rejectedTechnical.length > 0 || rejectedSupervisor.length > 0) ? `
                <div style="margin-bottom: 30px;">
                    <h2 style="color: #e74c3c; margin-bottom: 15px; font-size: 20px; padding-bottom: 10px; border-bottom: 1px solid #eee;">
                        <i class="fas fa-times-circle"></i> 无法参加的专家
                    </h2>
                    
                    ${rejectedTechnical.length > 0 ? `
                    <h3 style="color: #2c3e50; margin: 20px 0 10px; font-size: 18px;">技术组专家（${rejectedTechnical.length}位）</h3>
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; border: 1px solid #ddd;">
                        <thead style="background: #e74c3c; color: white;">
                            <tr>
                                <th style="padding: 8px; text-align: left; width: 10%;">姓名</th>
                                <th style="padding: 8px; text-align: left; width: 15%;">专家编号</th>
                                <th style="padding: 8px; text-align: left; width: 20%;">科室</th>
                                <th style="padding: 8px; text-align: left; width: 25%;">类型</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${rejectedTechnical.map((expert, index) => `
                                <tr style="${index % 2 === 0 ? 'background: #fdedec;' : ''}">
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.name}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee; color: #7f8c8d; font-size: 14px;">${expert.id}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.department}</td>
                                    <td style="padding: 8px; border-bottom: 1px solid #eee;">${expert.type}</td>
                    ...
                    <h3 style="color: #2c3e50; margin: 20px 0 10px; font-size: 18px;">监督组专家（${rejectedSupervisor.length}位）</h3>
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; border: 1px solid #ddd;">
                        <thead style="background: #e74c3c; color: white;">
                            <tr>
                                <th style="padding: 8px; text-align: left; width: 10%;">姓名</th>
                ...
                <!-- 页脚 -->
                <div style="margin-top: 50px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; color: #7f8c8d; font-size: 14px;">
                    <p>本报告由专家抽签系统自动生成</p>
        ...
        html2canvas(pdfContainer, {
            ...
            hideLoading();
            showNotification('error', 'PDF导出失败', '生成PDF文件时出错: ' + error.message);
            document.body.removeChild(pdfContainer);
        });
        ...
        hideLoading();
        showNotification('error', '导出失败', '生成PDF文件时出错: ' + error.message);
    }
}*/

// 使用html2canvas和jsPDF结合的方法，解决中文乱码问题
function exportDrawResultsToPDF() {
    showLoading('正在生成PDF文件...');
...
function generatePDFHTML(confirmedTechnical, confirmedSupervisor, rejectedTechnical, rejectedSupervisor) {
    return `
        ...
        '<span style="color: #f39c12; font-size: 12px;">(指定)</span>' : ''}
            ...
            <!-- 统计信息 -->
            <!--<div style="margin-top: 40px; padding: 25px; background: #f8f9fa; border-radius: 8px; border-top: 3px solid #3498db;">
                <h2 style="color: #2c3e50; margin: 0 0 20px 0; font-size: 20px;">统计信息</h2>
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                    <div style="padding: 15px; background: white; border-radius: 6px; border-left: 4px solid #3498db;">
                        <div style="font-size: 14px; color: #7f8c8d;">确认参加专家</div>
                        <div style="font-size: 24px; font-weight: bold; color: #27ae60;">
                            ${confirmedTechnical.length + confirmedSupervisor.length} 位
                        </div>
                    </div>
                    <div style="padding: 15px; background: white; border-radius: 6px; border-left: 4px solid #e74c3c;">
                        <div style="font-size: 14px; color: #7f8c8d;">无法参加专家</div>
                        <div style="font-size: 24px; font-weight: bold; color: #e74c3c;">
                            ${rejectedTechnical.length + rejectedSupervisor.length} 位
                        </div>
                    </div>
                </div>
